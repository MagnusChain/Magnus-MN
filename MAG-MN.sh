...
echo -e "${GREEN} ------- Magnus MASTERNODE INSTALLER V1.0.0--------+
 ...
 |        the masternode under a user Magnus.         |::
 |                                                  |::
...
cp -p -r magnus-cli /usr/local/bin

sudo mkdir ~/.magnus-params
cd ~/.magnus-params && wget https://github.com/MagnusChain/Magnus/raw/master/params/sapling-output.params && wget https://github.com/MagnusChain/Magnus/raw/master/params/sapling-spend.params

 magnus-cli stop
 sleep 5
 #Create datadir
 if [ ! -f ~/.magnus/magnus.conf ]; then
        sudo mkdir ~/.magnus
...
cd ~
clear
echo -e "${YELLOW}Creating magnus.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.magnus/magnus.conf
rpcuser=$rpcuser
...
EOF
    ...
    sudo chmod 755 -R ~/.magnus/magnus.conf
...
while true;do
    ...
    genkey=$(magnus-cli createmasternodekey)
    if [ "$genkey" ]; then
        break
    fi
sleep 7
...
EOF
    magnusd -daemon
#Finally, starting daemon with new magnus.conf
...
======================================================================== \a"
echo -e "${GREEN}MAG_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}MAG_mn1${NC} - with your desired masternode name (alias)
    ${GREEN}TxId${NC} - with Transaction Id from getmasternodeoutputs
    ${GREEN}TxIdx${NC} - with Transaction Index (0 or 1)
     Remember to save the masternode.conf and restart the wallet!
To introduce your new masternode to the Magnus network, you need to
...
========================================================================
To view masternode configuration produced by this script in magnus.conf:
${GREEN}cat ~/.magnus/magnus.conf${NC}
Here is your magnus.conf generated by this script:
-------------------------------------------------${GREEN}"
echo -e "${GREEN}MAG_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
cat ~/.magnus/magnus.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit magnus.conf, first stop the magnusd daemon,
then edit the magnus.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the magnusd daemon back up:
to stop:              ${GREEN}magnus-cli stop${NC}
to start:             ${GREEN}magnusd${NC}
to edit:              ${GREEN}nano ~/.magnus/magnus.conf${NC}
to check mn status:   ${GREEN}magnus-cli getmasternodestatus${NC}
========================================================================
